...
if cv2 is not None:
    from rgb_object_detection import detect_yellow_object


@unittest.skipIf(cv2 is None, "OpenCV is required (available on robot-pi)")
class YellowDetectionTests(unittest.TestCase):
    def test_detects_large_yellow_region(self) -> None:
        frame = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.rectangle(frame, (100, 120), (220, 260), (0, 255, 255), -1)
        result = detect_yellow_object(frame, (20, 100, 80), (38, 255, 255), 500)
        self.assertTrue(result.detected)
        self.assertGreaterEqual(result.confidence, 0.99)
        self.assertEqual(result.center_uv, (160, 190))

    def test_rejects_small_or_wrong_colour_regions(self) -> None:
